...
+ (instancetype)init {
    BKCKeyboard *view = [BKCKeyboard loadFirstNib:CGRectMake(0, ScreenHeight, ScreenWidth, ScreenWidth / 5 * 4 + kStatusBarHeight)];
    [view setHidden:YES];
    [view initUI];
    return view;
}
- (void)initUI {
    ...
    [self.nameLab setFont:kFont14];
    [self.nameLab setTextColor:kBlackColor];
    [self.markField setFont:kFont14];
    [self.moneyLab setFont:kFont18];
    [self.markField setFont:kFont14];
    [self.markField setTintColor:kColor_Main_Color];
    [self.markField setTextColor:kHexRGB(0x999999)];
    
    [self.textConstraintH setConstant:countcoordinatesX(60)];
    [self.keyConstraintB setConstant:kTabBarSpace];
    ...
    for (id obj in self.subviews) {
        if ([obj isKindOfClass:[UIButton class]] && [obj tag] >= 10) {
            ...
            if (btn.tag == FINISH_TAG) {
                ...
                [btn setBackgroundImage:[UIColor createImageWithColor:kWhiteColor] forState:UIControlStateNormal];
...
- (void)show {
    ...
    [UIView animateWithDuration:.3f delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        [self setTop:ScreenHeight - self.height];
